...
export default function BillingCard({ projectCredits, onCreditsUpdate }) {
  ...
  const handleClaimVoucher = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          data.error || `Error ${response.status}: Failed to claim voucher.`
        );
      }
      ...
      setMessage({
        type: 'success',
        text: data.message || 'Voucher claimed successfully!',
      });
  ...
  return (
    <div className="account-section billing-card">
      {' '}
      {/* Added billing-card class */}
      ...
      <div className="voucher-section">
        ...
        {message.text && (
          ...
          >
            {message.type === 'success' && (
              <CheckCircle className="w-5 h-5 flex-shrink-0 message-icon" />
            )}
            {message.type === 'error' && (
              <CloseIcon className="w-5 h-5 flex-shrink-0 message-icon" />
            )}
            {message.text}
          </div>
        )}
