...
export async function POST(request) {
  ...
  try {
    ...
    if (!voucher.is_active) {
      return NextResponse.json(
        { error: 'Voucher code is no longer active.' },
        { status: 410 }
      );
    }

    // 4. Check if Voucher Already Used
    const { data: usedVoucher, error: usedVoucherError } =
    ...
    if (usedVoucherError) {
      ...
      return NextResponse.json(
        { error: 'Failed to validate voucher status.' },
        { status: 500 }
      );
    }
    if (usedVoucher) {
    ...
    if (profileError || !profile) {
      ...
      return NextResponse.json(
        { error: 'Failed to update credits (profile not found).' },
        { status: 500 }
      );
    }
    const newCredits = (profile.credits || 0) + voucher.project_tier;
    ...
    if (updateError) {
      ...
      // Consider rollback if not using RPC
      return NextResponse.json(
        { error: 'Failed to update project credits.' },
        { status: 500 }
      );
    }
    // --- End Updates ---
